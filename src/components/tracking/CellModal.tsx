...
export function CellModal({ isOpen, onClose, cellData, onSaveRecord }: CellModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/75 backdrop-blur-md animate-fade-in">
      <div className="w-full max-w-md rounded-2xl glass-panel border border-white/15 bg-slate-950/95 p-6 shadow-2xl relative space-y-5">
        <button
          onClick={onClose}
        ...
        <div className="space-y-1 pr-6">
          <div className="flex items-center gap-2 flex-wrap">
            <span className="text-[10px] uppercase font-black tracking-wider px-2 py-0.5 rounded-full bg-indigo-500/20 text-indigo-300 border border-indigo-500/30">
              {task.type}
            </span>
          ...
          <div className="space-y-2">
            ...
            <div>
              ...
              <input
                ...
                className="w-full px-3.5 py-2 rounded-xl bg-white/5 border border-white/10 text-white text-sm focus:outline-none focus:border-indigo-500 transition"
              />
            </div>
          </div>
        )}
        ...
        <div>
          ...
          <textarea
            ...
            className="w-full px-3.5 py-2 rounded-xl bg-white/5 border border-white/10 text-white text-xs placeholder-slate-500 focus:outline-none focus:border-indigo-500 transition"
          />
        </div>
        ...
        <div className="pt-3 flex items-center justify-end gap-3 border-t border-white/10">
          ...
          <button
            ...
            onClick={handleSave}
            disabled={isSaving}
            className="px-5 py-2.5 rounded-xl gradient-primary text-white text-xs font-bold shadow-lg shadow-indigo-500/25 hover:opacity-95 transition disabled:opacity-50 active:scale-95"
          >
